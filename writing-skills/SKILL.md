...
- **This Claude Code harness only discovers direct children of `skills/`**
...
1. **Cross-cutting** (applies no matter which project/repo you're working
   ...
   itself) → top-level. This makes it visible to both tools.
2. **Specific to one repo** (hardcodes a DB, a user model, a URL, a build
   convention that only that repo uses) → nest under `<owner>/<project>/`,
   e.g. `genji1024/private-note/`. Only OpenCode will see it — that's fine,
   Claude Code sessions on unrelated repos don't need it anyway.
3. **Shared by several repos under one owner, but not universal** (e.g. a
   ...
   directly under `<owner>/`, a sibling of the project folders, not
