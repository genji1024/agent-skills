...
## Delegation Rules
 ...
 GitHub の Issue/PR コメントをサブエージェントに投稿させるときは、コメント本文の**テキスト作成は自分（オーケストレータ）で行う**。手順:

1. 自分でコメント本文を作成する
2. 本文をローカルファイル（例: `/tmp/comment-<purpose>.md`）に書き出す
3. サブエージェントには「そのファイルの内容を読んで GitHub に投稿する」ことだけを依頼する
4. テキスト作成をサブエージェントに丸投げしない（性能の低いサブエージェントにテキスト作成を委ねると、メンションの誤記や不自然な文章になる原因になる）

理由: g-ohara から「性能の低いサブエージェントにテキスト作成を丸投げしたことでメンション誤記等が発生した」との指摘を受け、運用ルール化した。
...
4. スキル変更は、このリポジトリ（agent-skills）の **main ブランチに直接コミット・プッシュする**。PR は作成しない。コミットメッセージは [commit-message](../../commit-message/SKILL.md) スキルに準拠する
5. 変更内容は、作業中の Issue や PR にコメントとして投稿し、**変更内容を反映したコミットへのリンク**を記載する（ファイルパスと変更概要も併記する）
