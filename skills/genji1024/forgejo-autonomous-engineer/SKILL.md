...
- **builder のローカルコミットが pusher からアクセス不可の場合がある（2026-08 実測）**: builder がローカルでコミットしても、pusher が external_directory 制約でローカルリポジトリのパスにアクセス不可の場合がある。この場合 push は「Everything up-to-date」となりローカルコミットが反映されない（実発生: `/home/genji/repos/genji1024/containerized-agents` でコミットを作成したが反映されなかった）。代替として `update_file`（または `create_file`）でリモートに直接コミットする。ただし 1ファイル=1コミットになるため、複数ファイル変更時はファイル毎にコミットされる点に注意。ローカルコミットの SHA とリモートに直接作成したコミットの SHA は異なるため、完了報告コメントには実際に push された（リモートに存在する）SHA を記載すること。
